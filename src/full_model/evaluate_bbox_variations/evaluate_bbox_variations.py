...
from src.full_model.evaluate_bbox_variations.custom_dataset_bbox_variations import (
    CustomDatasetBboxVariations,
)
...
path_to_partial_test_set = (
    "/u/home/tanida/datasets/dataset-with-reference-reports-partial-1000/test-1000.csv"
)
...
def compute_meteor_score(bbox_generated_sentences, bbox_reference_sentences):
    ...
    gen_sents, ref_sents = remove_gen_sents_corresponding_to_empty_ref_sents(
        bbox_generated_sentences, bbox_reference_sentences
    )
...
def get_bbox_features(model, images, bbox_coordinates):
    ...
    images, features = model.object_detector._transform_inputs_for_rpn_and_roi(
        images, features
    )
    image_shapes = images.image_sizes

    # bbox_roi_pool_feature_maps is a tensor of shape (batch_size * 29) x 2048 x H x W (where H = W = RoI feature map size)
    bbox_roi_pool_feature_maps = model.object_detector.roi_heads.box_roi_pool(
        features, bbox_coordinates, image_shapes
    )
...
def iterate_over_data_loader(test_loader, model, tokenizer):
    ...
    for num_batch, batch in tqdm(enumerate(test_loader)):
        ...
        bbox_coordinates = batch[
            "bbox_coordinates"
        ]  # List[torch.tensor], with len(list)=batch_size and each torch.tensor of shape 29 x 4
        bbox_reference_sentences = batch[
            "bbox_reference_sentences"
        ]  # List[str] of length (batch_size * 29)

        images = images.to(
            device, non_blocking=True
        )  # shape (batch_size x 1 x 512 x 512)
        bbox_coordinates = [
            bbox_coords.to(device, non_blocking=True)
            for bbox_coords in bbox_coordinates
        ]
        ...
        try:
            ...
            bbox_generated_sentences = tokenizer.batch_decode(
...
def get_data_loader(test_dataset):
    def collate_fn(batch: list[dict[str]]):
        ...
        batch_new = {}
        batch_new["images"] = (
            images_batch  # torch.tensor of shape batch_size x 1 x 512 x 512
        )
        batch_new["bbox_coordinates"] = (
            bbox_coordinates  # List[torch.tensor], with len(list)=batch_size and each torch.tensor of shape 29 x 4
        )
        batch_new["bbox_reference_sentences"] = (
            bbox_reference_sentences  # List[str] of length (batch_size * 29)
        )
...
def vary_bbox_coords_by_aspect_ratio(row):
    bbox_coords_single_image = row[
        "bbox_coordinates"
    ]  # List[List[int]] of shape 29 x 4
    bbox_widths_heights_single_image = row[
        "bbox_widths_heights"
    ]  # List[List[int]] of shape 29 x 2
    aspect_ratio_variations_bboxes = row[
        "aspect_ratio_variations"
    ]  # List[float] of len 29
    ...
    for bbox_coords, bbox_width_height, ratio_variation in zip(
        bbox_coords_single_image,
        bbox_widths_heights_single_image,
        aspect_ratio_variations_bboxes,
    ):
...
def vary_bbox_coords_by_scale(row):
    ...
    for bbox_coords, bbox_width_height, scale_factor in zip(
        bbox_coords_single_image,
        bbox_widths_heights_single_image,
        scale_variations_bboxes,
    ):
        x1, y1, x2, y2 = bbox_coords
        bbox_width, bbox_height = bbox_width_height
...
def vary_bbox_coords_by_position(row):
    ...
    relative_position_variation_bboxes = row[
        "relative_position_variations"
    ]  # List[List[float]] of shape 29 x 2
    image_width, image_height = row["image_width_height"]  # two integers

    # to store the new bbox coordinates after they have been varied
    varied_bbox_coords_single_image = []

    for bbox_coords, bbox_width_height, relative_position_variations in zip(
        bbox_coords_single_image,
        bbox_widths_heights_single_image,
        relative_position_variation_bboxes,
    ):
        x1, y1, x2, y2 = bbox_coords
        bbox_width, bbox_height = bbox_width_height
...
def vary_bbox_coords(variation_type, num_images, mean, std, test_set_as_df):
    if variation_type == "position":
        ...
        relative_position_variations = np.random.normal(
            mean, std, size=(num_images, 29, 2)
        )

        test_set_as_df["relative_position_variations"] = (
            relative_position_variations.tolist()
        )
        ...
        aspect_ratio_variations = np.exp(
            np.random.normal(mean, std, size=(num_images, 29))
        )
    ...
    test_set_as_df["bbox_coordinates_varied"] = test_set_as_df.apply(
        lambda row: variation_func(row), axis=1
    )
...
def evaluate_model_on_bbox_variations(variation_type, model, test_set_as_df, tokenizer):
    ...
    for std in stds_to_evaluate:
        ...
        test_dataset = CustomDatasetBboxVariations(
            dataset_as_df=test_set_as_df, transforms=transforms
        )
        test_loader = get_data_loader(test_dataset)

        bbox_generated_sentences, bbox_reference_sentences = iterate_over_data_loader(
            test_loader, model, tokenizer
        )
        ...
        with open(path_results_txt_file, "a") as f:
            f.write(
                f"{variation_type} variation, std {std}, meteor score: {meteor_score:.5f}\n"
            )
...
def get_test_set_as_df():
    ...
    test_set_as_df = pd.read_csv(
        path_to_partial_test_set, usecols=usecols, converters=converters
    )
    ...
    test_set_as_df["bbox_widths_heights"] = test_set_as_df.apply(
        lambda row: compute_bbox_widths_heights(row), axis=1
    )
    test_set_as_df["image_width_height"] = test_set_as_df.apply(
        lambda row: retrieve_image_widths_heights(row), axis=1
    )
...
def main():
    ...
    tokenizer = (
        get_tokenizer()
    )  # to decode (i.e. turn into human-readable text) the generated output ids by the language model

    for variation_type in ["scale", "aspect_ratio", "position"]:
        evaluate_model_on_bbox_variations(
            variation_type, model, test_set_as_df, tokenizer
        )
