...
def train(
    ...
    for epoch in range(NUM_EPOCHS):
        for i, data in enumerate(train_ld, 0):
            ...
            if (i + 1) % log_every == 0:
                print(
                    "epoch %d, iter %d, avg_loss %.3f, time_elapsed %.3f sec"
                    % (
                        epoch + 1,
                        i + 1,
            ...
            if (i + 1) % valid_niter == 0:
                ...
                for j in range(len(CONDITIONS)):
                    print(
                        "%s blank_f1:  %.3f, negation_f1: %.3f, uncertain_f1: %.3f, positive: %.3f"
                        % (
                            CONDITIONS[j],
                            metrics["blank"][j],
                            metrics["negation"][j],
                ...
                if metric_avg > best_metric:  # new best network
                    ...
                    path = os.path.join(
                        save_path, "model_epoch%d_iter%d" % (epoch + 1, i + 1)
                    )
                    torch.save(
                        {
                            "epoch": epoch + 1,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train BERT-base model on task of labeling 14 medical conditions."
    )
    parser.add_argument(
        "--train_csv",
        type=str,
