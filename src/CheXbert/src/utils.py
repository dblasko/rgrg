...
def compute_train_weights(train_path):
    ...
    for i in range(len(df)):  # loop over training set
        for cond in CONDITIONS:  # loop over all conditions
            ...
            weight_arr[i] += cond_weights[cond][
                label
            ]  # add weight for given class' label
...
def evaluate(model, dev_loader, device, f1_weights, return_pred=False):
    ...
    for j in range(len(y_pred)):
        cond = CONDITIONS[j]
        avg = weighted_avg(
            [negation_f1[j], uncertain_f1[j], positive_f1[j]], f1_weights[cond]
        )
        weighted.append(avg)
...
def test(model, checkpoint_path, test_ld, f1_weights):
    ...
    for j in range(len(CONDITIONS)):
        print(
            "%s blank_f1:  %.3f, negation_f1: %.3f, uncertain_f1: %.3f, positive_f1: %.3f"
            % (
                CONDITIONS[j],
                metrics["blank"][j],
                metrics["negation"][j],
    ...
    unc_macro_avg = np.mean(
        metrics["uncertain"][:-2]
    )  # No Finding, Support Devices have no uncertain labels in test set
    pos_macro_avg = np.mean(metrics["positive"])
    blank_macro_avg = np.mean(metrics["blank"])

    print(
        "blank macro avg: %.3f, negation macro avg: %.3f, uncertain macro avg: %.3f, positive macro avg: %.3f"
        % (blank_macro_avg, neg_macro_avg, unc_macro_avg, pos_macro_avg)
    )
    print()
    for j in range(len(CONDITIONS)):
